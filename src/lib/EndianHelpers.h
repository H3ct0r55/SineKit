...
inline constexpr bool kHostIsLE = (std::endian::native == std::endian::little);
...
template <IntWord T> [[nodiscard]] constexpr T byteswap(T v) noexcept {
...
template <IntWord T> [[nodiscard]] constexpr T le_to_host(T v) noexcept {
    return kHostIsLE ? v : byteswap(v);
}

template <IntWord T> [[nodiscard]] constexpr T be_to_host(T v) noexcept {
    return kHostIsLE ? byteswap(v) : v;
}

// ── Convert from host‑order to file‑order LE / BE ─────────────────────────
template <IntWord T> [[nodiscard]] constexpr T host_to_le(T v) noexcept {
    return kHostIsLE ? v : byteswap(v);
}

template <IntWord T> [[nodiscard]] constexpr T host_to_be(T v) noexcept {
    return kHostIsLE ? byteswap(v) : v;
}

// ── Streaming helpers (UNCHANGED SIGNATURES) ─────────────────────────────
//               write N‑byte integer/float in given endianness
template <IntWord T> inline void write_le(std::ofstream &os, T v) {
    v = host_to_le(v);
    os.write(reinterpret_cast<const char *>(&v), sizeof v);
}
template <IntWord T> inline void write_be(std::ofstream &os, T v) {
    v = host_to_be(v);
    os.write(reinterpret_cast<const char *>(&v), sizeof v);
}

//               read N‑byte integer/float in given endianness
template <IntWord T> [[nodiscard]] inline T read_le(std::ifstream &is) {
    T v{};
    is.read(reinterpret_cast<char *>(&v), sizeof v);
    return le_to_host(v);
}
template <IntWord T> [[nodiscard]] inline T read_be(std::ifstream &is) {
    T v{};
    is.read(reinterpret_cast<char *>(&v), sizeof v);
    return be_to_host(v);
}
...
[[nodiscard]] constexpr T swap_if_needed(T raw, Endian fileEndian) noexcept {
    if constexpr (sizeof(T) == 1)
        return raw; // never swap single byte
    return (fileEndian == Endian::Little) ? le_to_host(raw) : be_to_host(raw);
}
template <IntWord T>
[[nodiscard]] constexpr T host_to_file(T v, Endian fileEndian) noexcept {
    return (fileEndian == Endian::Little) ? host_to_le(v) : host_to_be(v);
}
